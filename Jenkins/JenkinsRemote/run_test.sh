...
docker-compose -f Jenkins/JenkinsRemote/docker-compose.yml -p $COMPOSE_ID rm -f
# Starting new stack environment
docker-compose -f Jenkins/JenkinsRemote/docker-compose.yml -p $COMPOSE_ID up --build